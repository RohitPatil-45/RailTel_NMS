...
function generateGraph() {
	...
	$.ajax({
		type: 'POST',
		url: 'nodeAvailabilityAverageGraph',
		dataType: 'json',
		...
		success: function(jsondata) {
			...
			Highcharts.chart('containervlx', {
				...
				series: [{
					...
					data: jsondata.map(item => item[2]), // Downtime Percent on y-axis
					...
					data: jsondata.map(item => item[1]), // Uptime Percent on y-axis
					showInNavigator: true,
					tooltip: {
						valueDecimals: 1,
						valueSuffix: '%'
