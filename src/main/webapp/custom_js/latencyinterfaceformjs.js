...
function groupDevices(option) {
	...
	var serviceUrl = base_url + "/interfaceReport/getGroupInterfaceIPDetails";
	$
			.ajax({
				type : 'get',
				dataType : 'json',
...
function groupDevices2(option) {
	var grpname = option.value;
	var l = window.location;
	var base_url = l.protocol + "//" + l.host + "/" + l.pathname.split('/')[1];
	var serviceUrl = base_url + "/interfaceReport/getInterfaceIPDetails";
	$
			.ajax({
				type : 'get',
				dataType : 'json',
